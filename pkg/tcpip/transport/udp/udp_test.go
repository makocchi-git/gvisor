...
func TestV4UnknownDestination(t *testing.T) {
	...
	testCases := []struct {
		...
		largePayload bool
		// checksumError if true, will set an invalid checksum in the
		// header.
		checksumError bool
	}{
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("flow:%s icmpRequired:%t largePayload:%t", tc.flow, tc.icmpRequired, tc.largePayload), func(t *testing.T) {
			...
			// todo
			c.injectPacket(tc.flow, payload, tc.checksumError)
			if !tc.icmpRequired {
...
// TestIncrementChecksumErrors verifies if a checksum error is detected,
// global and endpoint stats are incremented.
func TestIncrementChecksumErrors(t *testing.T) {
	for _, flow := range []testFlow{unicastV4, unicastV6} {
		...
		payload := newPayload()
		c.injectPacket(flow, payload, true)
