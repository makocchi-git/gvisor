...
func SendICMPError(r *stack.Route, eType header.ICMPv6Type, eCode header.ICMPv6Code, aux int, pkt *stack.PacketBuffer) bool {
	...
	if transport != nil && pkt.TransportProtocolNumber == header.ICMPv6ProtocolNumber {
		if transport.Size() < header.ICMPv6MinimumSize {
			return true
		}
		// in RFC 4443 all types less than 128 are defined as error packets.
		if header.ICMPv6(transport).Type() < 128 {
			return true
		}
	}
