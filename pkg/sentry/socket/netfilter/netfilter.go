...
func GetInfo(t *kernel.Task, stack *stack.Stack, outPtr usermem.Addr, ipv6 bool) (linux.IPTGetinfo, *syserr.Error) {
	...
	_, info, err := convertNetstackToBinary(stack, info.Name, ipv6)
	if err != nil {
...
func GetEntries4(t *kernel.Task, stack *stack.Stack, outPtr usermem.Addr, outLen int) (linux.KernelIPTGetEntries, *syserr.Error) {
	entries, err := getEntries(t, stack, outPtr, outLen, false)
	return *entries.(*linux.KernelIPTGetEntries), err
}

// GetEntries6 returns netstack's ip6tables rulesl.
func GetEntries6(t *kernel.Task, stack *stack.Stack, outPtr usermem.Addr, outLen int) (linux.KernelIP6TGetEntries, *syserr.Error) {
	entries, err := getEntries(t, stack, outPtr, outLen, true)
	return *entries.(*linux.KernelIP6TGetEntries), err
}

// getEntries returns either a linux.KernelIPTGetEntries or
// linux.KernelIP6TGetEntries depending on the ipv6 argument.
func getEntries(t *kernel.Task, stack *stack.Stack, outPtr usermem.Addr, outLen int, ipv6 bool) (interface{}, *syserr.Error) {
	...
	// tool can understand.
	entries, _, err := convertNetstackToBinary(stack, userEntries.Name, ipv6)
	if err != nil {
...
//
// Returns either a linux.KernelIPTGetEntries or linux.KernelIP6TGetEntries
// depending on the ipv6 argument.
func convertNetstackToBinary(stk *stack.Stack, tablename linux.TableName, ipv6 bool) (interface{}, linux.IPTGetinfo, error) {
	// The table name has to fit in the struct.
	if linux.XT_TABLE_MAXNAMELEN < len(tablename) {
		return nil, linux.IPTGetinfo{}, fmt.Errorf("table name %q too long", tablename)
	}

	table, ok := stk.IPTables().GetTable(tablename.String(), ipv6)
	if !ok {
		return nil, linux.IPTGetinfo{}, fmt.Errorf("couldn't find table %q", tablename)
	}
	...
	if ipv6 {
		entries := getEntries6(table, &info)
